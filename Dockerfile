...
# ----------------------------------------------------------------------
# Final Stage: imagen runtime ligera
# ----------------------------------------------------------------------
...
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
      libpq5 \
      netcat-openbsd \
    # Un ajuste: limpiar el cache de apt-get para reducir el tamaño final de la imagen
    && rm -rf /var/lib/apt/lists/*
...
# Copiar el código de la aplicación (¡Este es el paso esencial que faltaba para completar la app!)
# Usamos --chown para asignar propiedad al usuario no-root.
...
# CMD por defecto: Gunicorn (puedes sobreescribir en docker-compose para celery u otros procesos)
