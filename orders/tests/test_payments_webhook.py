...
from orders.models import Order, Payment, PaymentLog, ORDER_CONFIRMED
...
class PaymentWebhookTests(TestCase):
    ...
    def _post_payload(self, payload: dict, secret: str = "testsecret", headers=None):
        ...
        hdrs = {"HTTP_X_SIGNATURE": sig, "HTTP_X_PROVIDER": payload.get("provider", "mercadopago")}
        if headers:
            hdrs.update(headers)
        return self.client.post(WEBHOOK_URL, data=body, content_type="application/json", **hdrs)

    def test_webhook_confirms_payment_and_order(self):
        provider_ref = "prov-123"
        payload = {"id": provider_ref, "external_reference": str(self.order.id), "status": "approved", "amount": "10.00"}
    ...
    def test_webhook_idempotent_on_replay(self):
        ...
        self.assertEqual(payments.count(), 1, "Debe haber solo un objeto Payment creado (idempotencia)")
    ...
    def test_webhook_signature_invalid(self):
        ...
        resp = self.client.post(WEBHOOK_URL, data=body, content_type="application/json", HTTP_X_SIGNATURE=bad_sig)
