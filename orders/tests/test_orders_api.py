...
class OrderCreateTests(APITestCase):
    def setUp(self):
        ...
        self.cliente = User.objects.create_user(
            email="test_cliente@test.local",
            password="pw1234",
            full_name="Cliente Test",
            role=Role.objects.get(name="CLIENTE"),
        )

        self.feriante = User.objects.create_user(
            email="test_feriante@test.local",
            password="pw1234",
            full_name="Feriante Test",
            role=Role.objects.get(name="FERIANTE"),
        )

        self.feria = Feria.objects.create(nombre="Feria Test")
        self.puesto = Puesto.objects.create(
            feria=self.feria, feriante=self.feriante, nombre="Puesto Test"
        )
        self.producto = Producto.objects.create(
            puesto=self.puesto, nombre="Prod", precio=Decimal("2.00"), stock=5
        )
    ...
    def test_create_order_decrements_stock_and_creates_payment(self):
        ...
        payload = {
            "notas": "test",
            "items": [{"producto": str(self.producto.id), "cantidad": 2}],
        }
        resp = self.client.post(url, payload, format="json")
        self.assertEqual(
            resp.status_code,
            201,
            msg=f"Status code inesperado: {resp.status_code}, body: {getattr(resp, 'data', None)}",
        )
        ...
        # Verificar que se creó un pago asociado
...
class FerianteListTests(APITestCase):
    def setUp(self):
        ...
        prod = Producto.objects.create(
            puesto=puesto, nombre="P2Prod", precio=Decimal("1.00"), stock=10
        )

        self.client.force_authenticate(self.cliente)
        self.client.post(
            reverse("orders-list"),
            {"notas": "", "items": [{"producto": str(prod.id), "cantidad": 1}]},
            format="json",
        )
...
class RepartidorFlowTests(APITestCase):
    def setUp(self):
        ...
        self.repartidor = User.objects.create_user(
            ...
            password="pw",
            full_name="R3",
            role=Role.objects.get(name="REPARTIDOR"),
        )

        fer = Feria.objects.create(nombre="F3")
        puesto = Puesto.objects.create(
            feria=fer, feriante=self.repartidor, nombre="PuestoX"
        )
        prod = Producto.objects.create(
            puesto=puesto, nombre="PX", precio=Decimal("1.00"), stock=10
        )
    ...
    def test_claim_and_complete(self):
        ...
        claim = self.client.post(
            reverse("repartidor-orders-claim", args=[self.order_id])
        )
        ...
        complete = self.client.post(
            reverse("repartidor-orders-complete", args=[self.order_id])
        )
