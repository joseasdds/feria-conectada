...
def payment_webhook(request):
    ...
    if secret:
        ...
        if not hmac.compare_digest(received_sig, expected_sig):
            logger.warning("Invalid webhook signature received: %s (Expected prefix: %s)", received_sig, expected_sig[:8])
    ...
    logger.info(
        ...
        provider, provider_ref, external_reference, provider_status
    )
    ...
    try:
        from orders.models import Order, Payment, PaymentLog, ORDER_CONFIRMED, PAYMENT_STATUS_CHOICES
        PAYMENT_STATUS_VALUES = {k for k, _ in PAYMENT_STATUS_CHOICES}
    except ImportError:
        logger.error("Failed to import models (Order, Payment, etc.). Check orders.models.")
    ...
    try:
        ...
        payload_field = next((f for f in ['raw_payload', 'payload', 'data', 'raw', 'body'] if f in pl_field_names), None)
        if payload_field:
        ...
        if not log_created:
            logger.info("PaymentLog already exists for provider=%s provider_ref=%s: skipping (idempotent)", provider, provider_ref)
    ...
    try:
        ...
        if mapped_status not in PAYMENT_STATUS_VALUES:
            logger.warning("Mapped status %s not valid, defaulting to FAILED", mapped_status)
    ...
    try:
        with transaction.atomic():
            ...
            if external_reference:
                try:
                    order_obj = Order.objects.select_for_update().get(id=external_reference)
                except Order.DoesNotExist:
                    logger.warning("Order not found for external_reference=%s", external_reference)
                    order_obj = None
            ...
            try:
                ...
                logger.warning("IntegrityError on update_or_create for provider_ref=%s, retrying with get()", provider_ref)
                try:
                    ...
                    logger.warning("Payment disappeared after IntegrityError; re-creating.")
                    payment = Payment.objects.create(provider_ref=provider_ref, **defaults)
                    created = True
                except Exception:
                    logger.exception("Failed to fallback and retrieve/update payment %s", provider_ref)
                    raise

            logger.info(
                "Payment %s (provider_ref=%s) created=%s status=%s monto=%s",
                getattr(payment, 'id', None), provider_ref, created, payment.status, getattr(payment, 'monto', None)
            )
            ...
            if order_obj and mapped_status == 'SUCCESS':
                if order_obj.estado != ORDER_CONFIRMED:
                    ...
                    logger.info("Order %s estado actualizado a %s", order_obj.id, order_obj.estado)
                else:
                    logger.info("Order %s ya estaba en estado CONFIRMED. No se requiere actualización.", order_obj.id)

    except Exception:
        logger.exception("Unhandled exception while processing payment webhook for provider_ref=%s", provider_ref)
        return HttpResponse(status=500)
