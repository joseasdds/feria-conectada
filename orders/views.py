...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return response.Response(out_serializer.data, status=status.HTTP_201_CREATED, headers=headers)
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            for idx, item_data in enumerate(items_data):
                ...
                try:
                    producto = Producto.objects.select_for_update().get(id=prod_id, activo=True, puesto__activo=True)
                except Producto.DoesNotExist:
                    raise serializers.ValidationError({
                        'items': [{ 'producto': "Producto no encontrado o inactivo.", 'index': idx }]
                    })

                if producto.stock < cantidad:
                    raise serializers.ValidationError({
                        'items': [{ 'cantidad': f"Stock insuficiente para producto {producto.id}. Disponible: {producto.stock}", 'index': idx }]
                    })
