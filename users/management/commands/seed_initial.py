...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--admin-email', type=str, default=os.getenv('ADMIN_EMAIL', 'admin@local.test'))
        parser.add_argument('--admin-password', type=str, default=os.getenv('ADMIN_PASSWORD', None))
    ...
    def handle(self, *args, **options):
        ...
        if admin and skip_if_exists:
            self.stdout.write(self.style.WARNING("Superusuario existe, --skip-if-exists usado."))
        else:
            if not admin:
                # Intentamos create_user (si manager lo soporta)
                try:
                    admin = User.objects.create_user(email=admin_email, password=admin_password)
                except Exception:
                    admin = User(email=admin_email, full_name="Admin", role=admin_role, is_staff=True, is_active=True, is_superuser=True)
            ...
            admin.set_password(admin_password)
            admin.save()
            self.stdout.write(self.style.SUCCESS(f"Superuser creado/actualizado: {admin_email}"))
        ...
        if create_samples:
            ...
            for email, name, role_name in samples:
                ...
                if u and skip_if_exists:
                    self.stdout.write(self.style.WARNING(f"Usuario {email} existe, skip."))
                    continue
                if not u:
                ...
                u.role = role
                u.save()
                self.stdout.write(self.style.SUCCESS(f"Usuario creado/actualizado: {email} (role={role_name})"))