...
# 1. Definición de Variables (con valores por defecto si no están en .env o docker-compose)
DB_HOST="${DB_HOST:-}"
DB_PORT="${DB_PORT:-}"
...
WAIT_TIMEOUT="${WAIT_TIMEOUT:-60}"  # Tiempo máximo de espera para DB/Redis

# 2. Manejo de Señales (para apagado limpio de procesos)
term_handler() {
...
# 3. Función de Espera (Wait-for-it)
# Usa 'nc' (netcat) para comprobar si un puerto está abierto y listo.
wait_for_host() {
  local host="$1"
  local port="$2"
  local timeout="$3"
  ...
  # Verificamos si 'nc' existe, si no, intentamos instalarlo (para imágenes minimalistas)
  if ! command -v nc &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
      ...
      echo "ERROR: No se pudo instalar netcat. El script no podrá esperar la DB. Abortando."
      exit 1
    fi
  fi
...
# 4. Extracción de Host/Puerto de la URL de la Base de Datos
...
# 6. Esperar a Redis (Si REDIS_URL está definido)
if [ -n "$REDIS_URL" ]; then
  ...
  if ! wait_for_host "$REDIS_HOST" "$REDIS_PORT" "$WAIT_TIMEOUT"; then
    echo "WARNING: Redis $REDIS_HOST:$REDIS_PORT no respondió en ${WAIT_TIMEOUT}s. Continuando sin Redis."
  fi
fi
